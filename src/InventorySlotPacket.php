...
class InventorySlotPacket extends DataPacket implements ClientboundPacket{
	...
	protected function decodePayload(ByteBufferReader $in, int $protocolId) : void{
		...
		if($protocolId >= ProtocolInfo::PROTOCOL_1_26_20){
			...
			$this->storage = CommonTypes::readOptional($in, fn() => CommonTypes::getNetworkItemStackDescriptor($in, $protocolId));
	...
	protected function encodePayload(ByteBufferWriter $out, int $protocolId) : void{
		...
		if($protocolId >= ProtocolInfo::PROTOCOL_1_26_20){
			...
			CommonTypes::writeOptional($out, $this->storage, fn(ItemStackWrapper $w) => CommonTypes::putNetworkItemStackDescriptor($out, $w, $protocolId));
