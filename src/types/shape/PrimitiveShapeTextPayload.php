...
final class PrimitiveShapeTextPayload extends PrimitiveShapePayload{
	...
	public static function read(ByteBufferReader $in, int $protocolId) : self{
		...
		if ($protocolId >= ProtocolInfo::PROTOCOL_1_26_50) {
			$lineGapHeight = CommonTypes::readOptional($in, LE::readFloat(...));
		}
		$depthTest = CommonTypes::getBool($in);
		$showBackface = CommonTypes::getBool($in);
		$showTextBackface = CommonTypes::getBool($in);

		return new self($text, $useRotation, $backgroundColor, $lineGapHeight ?? null, $depthTest, $showBackface, $showTextBackface,);
	}
	...
	public function write(ByteBufferWriter $out, int $protocolId) : void{
		...
		if ($protocolId >= ProtocolInfo::PROTOCOL_1_26_50) {
			CommonTypes::writeOptional($out, $this->lineGapHeight, LE::writeFloat(...));
		}
