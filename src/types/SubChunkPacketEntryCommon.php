...
final class SubChunkPacketEntryCommon{
	...
	public static function read(ByteBufferReader $in, int $protocolId, bool $cacheEnabled) : self{
		...
		$data = $protocolId >= ProtocolInfo::PROTOCOL_1_26_40
			? (CommonTypes::getBool($in) ? CommonTypes::getString($in) : null)
			: (!$cacheEnabled || $requestResult !== SubChunkRequestResult::SUCCESS_ALL_AIR ? CommonTypes::getString($in) : "");

		$heightMapDataType = Byte::readUnsigned($in);
		$heightMapData = $protocolId >= ProtocolInfo::PROTOCOL_1_26_40
			? CommonTypes::readOptional($in, SubChunkPacketHeightMapInfo::read(...))
			: null;
		$heightMapData = match ($heightMapDataType) {
			SubChunkPacketHeightMapType::NO_DATA => null,
			SubChunkPacketHeightMapType::DATA => $protocolId >= ProtocolInfo::PROTOCOL_1_26_40
				? ($heightMapData ?? throw new PacketDecodeException("Expected heightmap data"))
				: SubChunkPacketHeightMapInfo::read($in),
		...
		if($protocolId >= ProtocolInfo::PROTOCOL_1_21_90){
			...
			$renderHeightMapData = $protocolId >= ProtocolInfo::PROTOCOL_1_26_40
				? CommonTypes::readOptional($in, SubChunkPacketHeightMapInfo::read(...))
				: null;
			$renderHeightMapData = match ($renderHeightMapDataType) {
				SubChunkPacketHeightMapType::NO_DATA => null,
				SubChunkPacketHeightMapType::DATA => $protocolId >= ProtocolInfo::PROTOCOL_1_26_40
					? ($renderHeightMapData ?? throw new PacketDecodeException("Expected render heightmap data"))
					: SubChunkPacketHeightMapInfo::read($in),
				SubChunkPacketHeightMapType::ALL_TOO_HIGH => SubChunkPacketHeightMapInfo::allTooHigh(),
				SubChunkPacketHeightMapType::ALL_TOO_LOW => SubChunkPacketHeightMapInfo::allTooLow(),
