...
class TextPacket extends DataPacket implements ClientboundPacket, ServerboundPacket{
	...
	protected function encodePayload(ByteBufferWriter $out, int $protocolId) : void{
		...
		switch($this->type){
			...
			/** @noinspection PhpMissingBreakStatementInspection */
			case self::TYPE_ANNOUNCEMENT:
				CommonTypes::putString($out, ($this->sourceName === '' && $protocolId >= ProtocolInfo::PROTOCOL_1_21_130) ? ' ' : $this->sourceName);
			case self::TYPE_RAW:
			case self::TYPE_TIP:
			case self::TYPE_SYSTEM:
			case self::TYPE_JSON_WHISPER:
			case self::TYPE_JSON:
			case self::TYPE_JSON_ANNOUNCEMENT:
				CommonTypes::putString($out, ($this->message === '' && $protocolId >= ProtocolInfo::PROTOCOL_1_21_130) ? ' ' : $this->message);
				break;
